...
class TrialsDataset(Dataset):
    """
    PyTorch Dataset wrapping generate_trials.
    Pre-generates one batch of trials and exposes (x, y, mask) per sample.
    """
    def __init__(self, rule, config, mode='random', noise_on=True):
        trial = generate_trials(rule, config, mode=mode, noise_on=noise_on)
        self.x = torch.tensor(trial.x, dtype=torch.float32)
        self.y = torch.tensor(trial.y, dtype=torch.float32)
        self.mask = torch.tensor(trial.c_mask, dtype=torch.float32)

...
class CustomRNN(nn.Module):
    def __init__(self, n_input, n_output):
        ...
        # Output layer
        self.W_out = nn.Linear(self.N, self.n_output, bias=False)

        # Bias
        #self.bias = nn.Parameter(torch.zeros(self.N))
        #self.bias = nn.Parameter(torch.zeros(self.N), requires_grad=False)
        self.register_buffer('bias', torch.zeros(self.N))

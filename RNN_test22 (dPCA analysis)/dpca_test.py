#!/usr/bin/env python3
"""
Animated dPCA scatter‑plot of RNN hidden states **without** temporal averaging.

This file is an **updated** version of the earlier `pca_animation_pointwise.py`.
It replaces the standard PCA pipeline with the _demixed_ PCA implementation
from the `dPCA` package.

Key updates (2025‑05‑25):
• Replaced `sklearn.decomposition.PCA` with `from dPCA.dPCA import dPCA`.
• Added `compute_dpca()` that runs dPCA on a dataset shaped as
  (neurons × time × trials) with only a time marginal (`labels='t'`).
• Down‑stream plotting code is unchanged – it still expects an array of shape
  (trials, T, K), so the dPCA output is rearranged accordingly.
• All other logic (GIF export, still export, live PyQt5 window) is untouched.

Run:
    python dpca_animation_pointwise.py

Dependencies: dPCA, torch, numpy, PyQt5, matplotlib, Pillow.
"""

import os, sys, json
import numpy as np
import torch
from dPCA.dPCA import dPCA        # ← NEW: demixed PCA implementation
...
ORDERS     = [0]
THRESHOLD  = 0.4
DPCA_NCOMP = 10              # number of dPCA components to keep (per marginal)
COLOUR_MAP = {
...
OUT_GIF  = f"dpca_pointwise_{START_MS}_{END_MS}_{COMP_DURS}_{ORDERS}.gif"
FPS      = 10
...
def build_dataset(model, hp):
    """Generate trials, align on int‑2 offset and return raw hidden states."""
    dt = hp["dt"]
...
def compute_dpca(h_raw, n_components=3, reg=None):
    """
    Parameters
    ----------
    h_raw : ndarray, shape (trials, T, N)
        Single-trial hidden states straight out of the RNN.
    Returns
    -------
    Z_rt : ndarray, shape (trials, T, n_components)
        Low-dimensional trajectories for every trial & time-step.
    """
    # Put neurons first, repeats second, time last → (N, R, T)
    X = np.transpose(h_raw, (2, 0, 1)).astype(np.float32)

    # Fit demixed PCA (r = repeat axis, t = time axis)
    dpca = dPCA(labels='rt', n_components=n_components, regularizer=reg)
    Zdict = dpca.fit_transform(X)          # keys: 'r', 't', 'rt'

    # The joint ‘rt’ marginal gives the full single-trial activity
    Z_rt = Zdict['rt'].transpose(1, 2, 0)  # (trials, T, K)
    return Z_rt
...
def save_gif(Z, colours, times, filename, fps):
    ...
    scat_f = ax.scatter(Z[false_idx, 0, 0], Z[false_idx, 0, 1],
                        c=[colours[i] for i in false_idx], s=15, alpha=0.8)
    ax.set(xlabel="dPC‑1", ylabel="dPC‑2",
           xlim=(Z[:, :, 0].min() * 1.1, Z[:, :, 0].max() * 1.1),
           ylim=(Z[:, :, 1].min() * 1.1, Z[:, :, 1].max() * 1.1))
...
class DPCAScatterWindow(QMainWindow):
...
def main():
    ...
    win = DPCAScatterWindow(Z_sub, colours, times_sub)
    win.setWindowTitle(f"Animated dPCA scatter (N = {Z_sub.shape[0]})")
    win.resize(800, 600)
    win.show()
    sys.exit(app.exec_())

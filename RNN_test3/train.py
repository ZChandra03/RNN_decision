...
PyTorch re‑implementation of the TensorFlow/Keras code in main.py, network.py and train.py.
It preserves the high‑level API (run() → train()) and file layout while translating the
model definition, dataset pipeline and training loop to pure PyTorch.

Key replacements
----------------
* BioRNN            →  nn.Module that iterates through time steps with Dale's law & autapse mask
* CustomDataset     →  torch.utils.data.Dataset (IntervalDataset) adding on‑the‑fly input noise
* build_model()     →  returns RNNModel(BioRNN → Linear) on CUDA if available
* train()           →  manual epoch loop with gradient clipping & early stop on hp['target_loss']
* run()             →  mirrors main.py experiment sweep

The script expects that `task.py` (unchanged) provides `generate_trials()` returning
x, y, c_mask with shapes (batch, T, n_features).
"""
...
# ------------- external dependency -----------------------------
# Keep your existing task.py; only import here.
...
def train(seed_name, rule, seed, checkpoint_suffix='', hp_override=None, reload_directory=None):
    # ----------------------- IO setup ------------------------------------
    model_dir = os.path.join('models', seed_name)
    ...
    for epoch in range(hp['epochs']):
        ...
        for x_batch, y_batch, c_mask_batch in dataloader:
            ...
            c_mask_batch = c_mask_batch.to(DEVICE)[:, :, 0:1]  # ensure broadcastable dims

            opt.zero_grad()
            y_hat = model(x_batch)
            loss = mse_loss_with_mask(y_hat, y_batch, c_mask_batch)
            loss.backward()

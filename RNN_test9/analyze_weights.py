...
SAVE_PATH = os.path.join(OUTPUT_DIR, "trial_activation_figure.png")  # output figure file
...
def plot_condition(ax_heat, ax_out, model, hp, comp_ms: int):
    ...
    #h_sorted = sort_by_peak(h_norm)
    h_sorted = sort_by_peak(h)
...
if __name__ == "__main__":
    ...
    # grab its ticks and turn the top tick into ">1"
    ticks = cbar.get_ticks()                  # e.g. array([0. , 0.5, 1. ])
    # build default labels (you can adjust formatting as you like)
    labels = [f"{t:.1f}" for t in ticks]      # ['0.0','0.5','1.0']
    # replace the last (i.e. max) label with ">1"
    labels[-1] = ">1"
    cbar.set_ticks(ticks)                     # re‑set the same tick locations

...
def train(seed_name, rule, seed, checkpoint_suffix='', hp_override=None, reload_directory=None):
    ...
    for epoch in range(hp['epochs']):
        ...
        for x_batch, y_batch, c_mask_batch in dataloader:
            ...
            # ── L2 rate penalty ──
            # Run the RNN alone to get hidden rates (batch, T, N)
            h = model.rnn(x_batch)
            rate_penalty = hp['L2_activity'] * (h**2).mean()
            loss = loss + rate_penalty

            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
            with torch.no_grad():
                model.rnn.w_rec.clamp_(min=0.0)                   # force w_rec ≥ 0
...
def run():
    ...
    for exp in range(numExp):
        ...
        for rule in rule_list:
            seed_name = f'seed_{SeedBase + exp}'
